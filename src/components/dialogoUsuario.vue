                    ...
                    <q-card-section class="q-pb-xs">
                        ...
                        <div class="col-12 row q-col-gutter-sm" v-if="parMostrarBarraBusqueda==1">
                            ...
                            <div class="col-12 col-sm-2">
                              <q-input label="N° de Documento" v-model="objFiltro.nroDocumento" v-on:keyup.enter="buscar" dense outlined/>
                            </div>

                            <div class="col-12 col-sm-6">
                              <q-input label="Apellidos y nombres" v-model="objFiltro.apellidosNombres" v-on:keyup.enter="buscar" dense outlined/>
                            </div>

                            <div class="col-12 col-sm-4">
                                <q-input label="Nombre de usuario" v-model="objFiltro.usuLdap" v-on:keyup.enter="buscar" :autofocus="true" dense outlined/>
                            </div>

                            <div class="col-12 col-sm-8">
                                <q-input label="Unidad organica" v-model="objFiltro.unidadOrganica" v-on:keyup.enter="buscar" dense outlined/>
                            </div>

                            <div class="col-12 col-sm-2">
                                <q-select label="Es director" v-model="objFiltro.esDirector" :options="optionsDirector" option-label="nombre" option-value="id" emit-value map-options outlined dense denseOpts />
                            </div>
                        ...
                        <div class="row q-col-gutter-sm col-12">
                            <div class="col-12">
                                <q-table :data="lista" :columns="columnas" id="idCargo"
                                    ...
                                    <template v-slot:body-cell-flgDirector="scope">
                                        <q-td :props="scope" auto-width style="width: 15px">
                                            <q-btn icon="check" size="12px" color="green" v-if="scope.row.flgDirector=='1'" dense flat round >
                                                <q-tooltip anchor="center left" self="center right" :offset="[10, 10]">Es director</q-tooltip>
                                            </q-btn>
                                        </q-td>
                                    </template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                objFiltro: { nroDocumento: '', apellidosNombres: '', usuLdap: '', idUnidadOrganica: null, esDirector: 'SF' },
                dialogo: false,
                dialogoFormulario: false,
                obj: { nombre: this.parValorInputDefecto },
                optionsDirector: [ { id: 'SF', nombre:'TODOS' }, { id: '1', nombre:'SI' }, { id: '0', nombre:'NO' } ],

                lista: [],
                filter: '',
                paginacion: {rowsPerPage: 10},
                opcionesFilasPorPagina: [5,10,20,50,100],
                columnas: [
                    { name: 'flgDirector', label: 'DIRECTOR', field: row => row.flgDirector, align: 'left' },
                    { name: 'usuLdap', label: 'USUARIO', field: row => row.usuLdap, align: 'left' },
                    { name: 'nroDocumento', label: 'N° DE DOCUMENTO', field: row => row.nroDocumento, align: 'left' },
                    { name: 'apellidosNombres', label: 'APELLIDOS Y NOMBRES', field: row => row.apellidosNombres, align: 'left' },
                    { name: 'unidadOrganica', label: 'UNIDAD FUNCIONAL', field: row => row.unidadOrganica, align: 'left' },
                    { name: 'cargo', label: 'CARGO', field: row => row.cargo, align: 'left' },
        ...
        methods: {
            ...
            buscar: function() {
                ...
                if ( this.parTipoBusqueda=='PARAMETRO' ) {
                    ...
                    let nroDocumento = this.objFiltro.nroDocumento=='' ? 'SF' : this.objFiltro.nroDocumento;
                    let apellidosNombres = this.objFiltro.apellidosNombres=='' ? 'SF' : this.objFiltro.apellidosNombres;
                    let usuLdap = this.objFiltro.usuLdap=='' ? 'SF' : this.objFiltro.usuLdap;
                    let flgDirector = this.objFiltro.esDirector;
                    let idUnidadOrganica = 'SF';
                    let comodin = 'SF';

                    if ( !(nroDocumento!=='SF' || apellidosNombres!=='SF' || usuLdap!=='SF' || idUnidadOrganica!=='SF' || flgDirector!=='SF') ) {
                        this.cargando = false;
                        alert('Use algunos de los filtros para realizar la busqueda');
                        return;
                    }

                    valor = idUsuario+';'+usuLdap+';'+nroDocumento+';'+apellidosNombres+';'+idUnidadOrganica+';'+flgDirector+';'+comodin;
                }

              let form = new FormData();
              form.append('obj', JSON.stringify({ tipo: this.parTipo, valor: valor }));

              this.$axios.post(`${process.env.API_URL}/STsUsuario?accion=LISTAR`, form, {headers: {'Content-Type': 'multipart/form-data'}
              }).then(response => {
            ...
            filaSeleccionada: function(evt, row, index, tipoRespuesta){
              ...
              this.obj.nombre = tipoRespuesta=='1' ? row.usuLdap + ' (' + row.apellidosNombres + ' - ' + row.unidadOrganica + ( row.flgDirector == '1' ? ' - Director/a' : '' ) + ')' : '';
